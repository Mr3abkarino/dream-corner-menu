...
// 🔥🔥🔥 صمام أمان حل مشكلة الكاش: رقم إصدار المنيو الحالي 🔥🔥🔥
// كل ما تعدل أسعار أو تضيف أصناف في المستقبل، غير الرقم ده (مثلاً خليه "1.0.2")
// المتصفح عند الزبون هيمسح الكاش القديم فوراً ويقرأ التحديثات أوتوماتيك كأنه أول مرة يدخل!
...
export default function RestaurantMenu() {
  ...
  // 🔥 تأثير فحص رقم الإصدار وتصفير الكاش القديم أوتوماتيكياً لحل مشكلتك 100% 🔥
  useEffect(() => {
    (async () => {
      try {
        if (typeof window !== "undefined" && window.localStorage) {
        ...
        <div className="max-w-3xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3 min-w-0">
            <div onClick={handleLogoClickLocal} className="cursor-pointer active:scale-95 transition-transform shrink-0 relative">
              ...
              {logoClicks > 0 && <span className="absolute -top-1.5 -right-1.5 w-5 h-5 bg-red-600 text-white rounded-full flex items-center justify-center text-[10px] font-bold animate-pulse">{logoClicks}</span>}
            </div>
            <div className="min-w-0">
