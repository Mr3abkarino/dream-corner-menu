    ...
    saveTimer.current = setTimeout(async () => {
      try {
        await safeStorage.set("dream-corner-menu", JSON.stringify({
          items, restaurantName, tagline, address, menuUrl, whatsappNumber, vodafoneCash, instapay, adminPin, themeId: theme.id,
        }));
      } catch (e) {}
    }, 500);
            ...
            <div className="grid grid-cols-1 gap-2.5">
              {THEMES.map((t) => (
                <button key={t.id} onClick={() => { setTheme(t); setThemePickerOpen(false); }} className="flex items-center gap-3 p-3 rounded-xl border text-right transition-colors hover:bg-black/5" style={{ borderColor: (theme.muted || "#B3A18C") + "30", background: t.bg }}>
                  <div className="flex gap-1 shrink-0"><span className="w-6 h-6 rounded-full border border-white/20" style={{ background: t.accent }} /><span className="w-6 h-6 rounded-full border border-white/20" style={{ background: t.accent2 }}px /></div>
                      ...
                      {editingId === item.id ? (
                        ...
                        <div className="flex items-center justify-between gap-2">
                          ...
                          <div className="flex items-center gap-1 shrink-0">
                            <button onClick={() => setEditingId(item.id)} className="p-2 rounded-full border" style={{ borderColor: theme.muted + "40" }}><Pencil size={13} /></button>
                            <button onClick={() => deleteItem(item.id)} className="p-2 rounded-full border" style={{ borderColor: theme.muted + "40" }}><Trash2 size={13} /></button>
                          </div>
                        </div>
                      )}
                    </div>
