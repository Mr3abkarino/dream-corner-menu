...
const LOGO_SRC = "data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'><rect width='100' height='100' rx='50' fill='%23D4AF37'/><text y='0.72em' x='0.18em' font-size='65'>🍕</text></svg>";
  ...
  return (
    ...
    >
      ...
      <header
        ...
        style={{ background: theme.bg + "E6", borderColor: (theme.muted || "#B3A18C") + "30" }}
      >
        <div className="max-w-3xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2.5 min-w-0">
            <div onClick={handleLogoClick} className="cursor-pointer active:scale-95 transition-transform shrink-0 relative">
              <img
                ...
                className="w-10 h-10 rounded-full object-contain border border-white/20"
                style={{ padding: 1 }}
              />
              {logoClicks > 0 && (
                <span className="absolute -top-1.5 -right-1.5 w-5 h-5 bg-red-600 text-white rounded-full flex items-center justify-center text-[9px] font-bold animate-ping">
                  {logoClicks}
                </span>
              )}
            </div>
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            {isAdmin ? (
              ...
              <a href={"tel:" + whatsappNumber} className="p-2 rounded-full border transition-transform active:scale-95" style={{ borderColor: (theme.muted || "#B3A18C") + "40" }} aria-label="اتصل بنا">
                <Phone size={17} />
              </a>
            )}
          </div>
            ...
            <button
              ...
              style={
                activeCat === c
                  ...
                  : { borderColor: (theme.muted || "#B3A18C") + "40", color: theme.muted }
              }
            >
              {c}
            </button>
        ...
        <div className="relative">
          <input
            ...
            style={{
              ...
              borderColor: (theme.muted || "#B3A18C") + "40",
              color: theme.text
            }}
          />
                        ...
                        {item.sizes.map((sz) => {
                          ...
                          return (
                            <div
                              ...
                              style={{ borderColor: (theme.muted || "#B3A18C") + "30", background: theme.surface2 }}
                            >
                              ...
                              {qty > 0 ? (
                                <div className="flex items-center gap-1.5 mr-1.5">
                                  ...
                                  <button onClick={() => addToCart(key, -1)} className="w-5 h-5 rounded-full flex items-center justify-center border" style={{ borderColor: (theme.muted || "#B3A18C") + "50" }}>
                                    <Minus size={11} />
                                  </button>
                                </div>
                              ) : (
                      ...
                      <div className="flex justify-end -mt-6">
                        {cart[item.id] > 0 ? (
                          <div className="flex items-center gap-2 rounded-full px-1.5 py-1" style={{ background: theme.surface2 }}>
                            ...
                            <button onClick={() => addToCart(item.id, -1)} className="w-7 h-7 rounded-full flex items-center justify-center border" style={{ borderColor: (theme.muted || "#B3A18C") + "50" }}>
                              <Minus size={14} />
                            </button>
                          </div>
                        ) : (
      ...
      <div
        ...
        style={{ background: theme.bg + "F5", borderColor: (theme.muted || "#B3A18C") + "25", color: theme.muted, backdropFilter: "blur(8px)" }}
      >
                  ...
                  {cartList.map((cartItem) => (
                    <div key={cartItem.key} className="flex items-center justify-between gap-2 border-b pb-2" style={{ borderColor: (theme.muted || "#B3A18C") + "20" }}>
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <button onClick={() => addToCart(cartItem.key, -1)} className="w-7 h-7 rounded-full border flex items-center justify-center" style={{ borderColor: (theme.muted || "#B3A18C") + "50" }}>
                          <Minus size={14} />
                        </button>
                        <span className="w-4 text-center font-bold text-sm">{cartItem.qty}</span>
                        <button onClick={() => addToCart(cartItem.key, 1)} className="w-7 h-7 rounded-full flex items-center justify-center" style={{ background: theme.accent, color: theme.bg }}>
                          <Plus size={14} />
                        </button>
                      </div>
                    </div>
            ...
            <form onSubmit={handleVerifyPin} className="space-y-4">
              ...
              <label className="block text-sm space-y-1">
                ...
                <input
                  ...
                  style={{ borderColor: (theme.muted || "#B3A18C") + "40", color: theme.text }}
                  required
                />
              </label>
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  style={{ borderColor: (theme.muted || "#B3A18C") + "40" }}
                >
                  إلغاء
                </button>
              </div>
          ...
          <Sheet theme={theme} title="إعدادات الإدارة والأسعار" onClose={() => setAdminOpen(false)}>
            <div className="space-y-4 max-h-[65vh] overflow-y-auto pr-1">
              ...
              <div className="pt-4 border-t" style={{ borderColor: (theme.muted || "#B3A18C") + "30" }}>
                ...
                <div className="space-y-3">
                  {items.map((item) => (
                    <div key={item.id} className="p-3 rounded-xl border transition-all" style={{ borderColor: (theme.muted || "#B3A18C") + "25", background: theme.surface }}>
                      {editingId === item.id ? (
                        <div className="space-y-2">
                          <input value={item.name} onChange={(e) => updateItem(item.id, { name: e.target.value })} className="w-full px-2.5 py-1.5 rounded-lg border bg-transparent text-sm font-bold" style={{ borderColor: (theme.muted || "#B3A18C") + "40", color: theme.text }} placeholder="اسم الوجبة" />
                          <input value={item.desc || ""} onChange={(e) => updateItem(item.id, { desc: e.target.value })} className="w-full px-2.5 py-1.5 rounded-lg border bg-transparent text-xs" style={{ borderColor: (theme.muted || "#B3A18C") + "40", color: theme.text }} placeholder="وصف المكونات" />
                          <div className="grid grid-cols-2 gap-2">
                            <input value={item.cat} onChange={(e) => updateItem(item.id, { cat: e.target.value })} className="px-2 py-1.5 rounded-lg border bg-transparent text-xs" style={{ borderColor: (theme.muted || "#B3A18C") + "40", color: theme.text }} placeholder="القسم الرئيسي" />
                            <input value={item.subcat || ""} onChange={(e) => updateItem(item.id, { subcat: e.target.value })} className="px-2 py-1.5 rounded-lg border bg-transparent text-xs" style={{ borderColor: (theme.muted || "#B3A18C") + "40", color: theme.text }} placeholder="القسم الفرعي" />
                          </div>
                            ...
                            <div className="space-y-1.5">
                              {item.sizes.map((sz, idx) => (
                                <div key={idx} className="flex items-center gap-2">
                                  <input value={sz.label} onChange={(e) => updateSize(item.id, idx, { label: e.target.value })} className="w-20 px-2 py-1.5 rounded-lg border bg-transparent text-xs text-center" style={{ borderColor: (theme.muted || "#B3A18C") + "40", color: theme.text }} placeholder="الحجم" />
                                  <input type="number" value={sz.price} onChange={(e) => updateSize(item.id, idx, { price: Number(e.target.value) })} className="flex-1 px-2 py-1.5 rounded-lg border bg-transparent text-xs" style={{ borderColor: (theme.muted || "#B3A18C") + "40", color: theme.text }} placeholder="السعر" />
                                </div>
                              ))}
                            </div>
                          ) : (
                            <input type="number" value={item.price} onChange={(e) => updateItem(item.id, { price: Number(e.target.value) })} className="w-full px-2 py-1.5 rounded-lg border bg-transparent text-xs" style={{ borderColor: (theme.muted || "#B3A18C") + "40", color: theme.text }} placeholder="السعر بالجنيه المصري" />
                          )}
                        ...
                        <div className="flex items-center justify-between gap-2">
                          ...
                          <div className="flex items-center gap-1 shrink-0">
                            <button onClick={() => setEditingId(item.id)} className="p-2 rounded-full border transition-all hover:bg-black/10" style={{ borderColor: (theme.muted || "#B3A18C") + "40" }}>
                              <Pencil size={13} />
                            </button>
                            <button onClick={() => deleteItem(item.id)} className="p-2 rounded-full border transition-all hover:bg-red-500/10 hover:border-red-500/40" style={{ borderColor: (theme.muted || "#B3A18C") + "40" }}>
                              <Trash2 size={13} />
                            </button>
                          </div>
                        </div>
  ...
  return (
    <div className="relative z-10 w-full md:max-w-md max-h-[85vh] rounded-t-3xl md:rounded-3xl p-5 overflow-y-auto" style={{ background: theme.bg, color: theme.text, border: "1px solid " + (theme.muted || "#B3A18C") + "30" }} dir="rtl">
      <div className="flex items-center justify-between mb-4 pb-2 border-b" style={{ borderColor: (theme.muted || "#B3A18C") + "20" }}>
        ...
        <button onClick={onClose} className="p-1.5 rounded-full border" style={{ borderColor: (theme.muted || "#B3A18C") + "40" }}>
          <X size={15} />
        </button>
      </div>
      {children}
...
// التصدير الافتراضي في سطر منفصل لحل مشكلة Rollup AST بشكل نهائي وقاطع
export default App;
